...
class CapstoneTestCase(unittest.TestCase):
    def setUp(self):
        ...
        setup_db(self.app)
        self.sample_movie = {
            'title': 'Diehard', 'release_date': '07/15/1988'
        }

        self.sample_actor = {
            'name': 'Bruce Willis', 'age': '32', 'gender': 'male'
        }
    ...
    def test_failed_post_actor(self):
        res = self.client().post(
            '/actors', headers=self.header_producer)  # no json for creation
        data = json.loads(res.data)
    ...
    def test_patch_actor(self):
        ...
        res = self.client().patch('/actors/' + str(sample.id),
                                  json={'name': 'Walter Bruce Willis'},
    ...
    def test_failed_patch_actor(self):
        sample = Actor.query.first()
        res = self.client().patch('/actors/' + str(sample.id),
                                  headers=self.header_director)  # no name
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 400)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], 'bad request')
    ...
    def test_succesful_delete_actor(self):
        ...
        res = self.client().delete('/actors/' +
                                   str(sample.id), headers=self.header_director)
        data = json.loads(res.data)
        actor = Actor.query.filter(
            Actor.id == sample.id).one_or_none()  # 1 per id, or none
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_failed_delete_actor(self):
        res = self.client().delete(
            '/actors/9999999999',
            headers=self.header_producer)  # no such actor
        data = json.loads(res.data)
    ...
    def test_failed_post_movie(self):
        res = self.client().post(
            '/movies', headers=self.header_producer)  # no json for creation
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 400)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], 'bad request')
    ...
    def test_patch_movie(self):
        ...
        res = self.client().patch('/movies/' + str(sample.id),
                                  json={'name': 'Diehard.'}, headers=self.header_producer)
    ...
    def test_failed_patch_movie(self):
        sample = Movie.query.first()
        res = self.client().patch('/movies/' + str(sample.id),
                                  headers=self.header_producer)  # no title
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 400)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], 'bad request')
    ...
    def test_succesful_delete_movie(self):
        ...
        res = self.client().delete('/movies/' +
                                   str(sample.id), headers=self.header_producer)
        data = json.loads(res.data)
        movie = Movie.query.filter(
            Movie.id == sample.id).one_or_none()  # 1 per id, or none
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertEqual(data['deleted_id'], sample.id)
    ...
    def test_failed_delete_movie(self):
        res = self.client().delete(
            '/movies/999999999999',
            headers=self.header_producer)  # no such movie
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 404)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], 'not found')
    ...
    def test_no_permissions_for_action(self):
        sample = Actor.query.first()  # delete first result for test
        res = self.client().delete('/actors/' +
                                   str(sample.id), headers=self.header_assistant)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 401)
